...
const updateFavorite = () => {
  ...
  if (isFavorite.value) {
    clusterSettings.favoriteNamespaces =
      clusterSettings.favoriteNamespaces.filter(
        (favorite: string) => favorite !== props.namespace,
      );
  } else {
...
<template>
  <div class="flex items-center justify-between w-full">
    <n-icon size="20" class="group mr-2" @click.stop="updateFavorite">
      <Star16Regular
        :class="{ 'block group-hover:hidden': !isFavorite, hidden: isFavorite }"
      />
      <Star16Filled
        :class="{ block: isFavorite, 'hidden group-hover:block': !isFavorite }"
      />
    </n-icon>
    <span>{{ namespace }}</span>
  </div>
