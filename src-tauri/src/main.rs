...
#[tauri::command]
async fn list_pods(context: &str, namespace: &str, label_selector: &str, field_selector: &str) -> Result<Vec<Pod>, SerializableKubeError> {
    let client = client_with_context(context).await?;
    let pod_api: Api<Pod> = Api::namespaced(client, namespace);

    return pod_api
        .list(&ListParams::default().labels(label_selector).fields(field_selector))
        .await
        .map(|pods| pods.items)
        .map_err(|err| SerializableKubeError::from(err));
}
